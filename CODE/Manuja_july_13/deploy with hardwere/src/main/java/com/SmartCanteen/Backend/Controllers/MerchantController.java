...
public class MerchantController {
    ...
    public ResponseEntity<?> updateCustomerBiometrics(
            @RequestBody CustomerBiometricUpdateDTO dto,
            @RequestHeader("Authorization") String authHeader) { // ✨ Added authHeader

        // Extract token from the Authorization header
        String token = authHeader.startsWith("Bearer ") ? authHeader.substring(7) : authHeader;

        // Call the service method with both arguments
        merchantService.updateCustomerBiometricsByEmail(dto, token); // ✨ Passed token
