...
public class FoodCategoryService {
    ...
    public List<FoodCategoryDTO> getAllCategories() {
        List<FoodCategory> categories = foodCategoryRepository.findAll(Sort.by(Sort.Direction.ASC, "id"));
    ...
    // In FoodCategoryService.java
    public List<FoodCategoryDTO> getAllCategoriesWithTodayMenu() {
        List<Long> todaysMenuIds = todaysMenuRepository.findByDate(LocalDate.now())
                .stream().map(item -> item.getMenuItem().getId()).collect(Collectors.toList());

        List<FoodCategory> categories = foodCategoryRepository.findAll();
        List<FoodCategoryDTO> result = new ArrayList<>();
        for (FoodCategory cat : categories) {
            FoodCategoryDTO dto = mapToDTOWithItems(cat);
            for (MenuItemDTO item : dto.getMenuItems()) {
                item.setIsInTodayMenu(todaysMenuIds.contains(item.getId()));
            }
            result.add(dto);
        }
        return result;
