...
const String authEndpoint = "/api/biometric/confirm";  // ✅ Updated
...
// Payment wait
bool waitingForPaymentStatus = false;
...
void setup() {
  ...
  // FIXED: Improved paymentStatus endpoint
  server.on("/paymentStatus", HTTP_POST, []() {
    if (!server.hasArg("plain")) {
    ...
    String body = server.arg("plain");
    Serial.println("📨 Payment Status JSON:");
    Serial.println(body);
    ...
    if (!doc.containsKey("status") || !doc.containsKey("message") || !doc.containsKey("orderId")) {
...
void loop() {
  ...
  if (waitingForPaymentStatus) {
    if (millis() - paymentStatusTime > PAYMENT_STATUS_TIMEOUT) {
      lcd.clear();
  ...
  if (isVerifying) {
    ...
    if (fid > 0) {
      if (finger.confidence >= MIN_CONFIDENCE) {
        ...
        waitingForPaymentStatus = true;
        paymentStatusTime = millis();
        delay(2000);
        lcd.clear();
