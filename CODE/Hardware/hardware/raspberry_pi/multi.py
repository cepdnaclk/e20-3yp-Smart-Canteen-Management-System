...
biometric_queue = queue.Queue()  # Queue for biometric requests
server_lock = threading.Lock()  # Lock to manage server start/stop
...
welcome_title = tk.Label(welcome_frame, text="🍽️ Smart Canteen", font=('Arial', 24, 'bold'), fg="#00d4aa", bg="#1a1a2e")
welcome_title.pack(pady=40)
welcome_subtitle = tk.Label(welcome_frame, text="Tap your RFID card to start", font=('Arial', 14), fg="#ffffff", bg="#1a1a2e")
welcome_subtitle.pack(pady=10)
loading_label = tk.Label(welcome_frame, text="💳 Ready to scan...", font=('Arial', 12), fg="#00d4aa", bg="#1a1a2e")
...
name_label = tk.Label(customer_info, textvariable=name_var, font=('Arial', 14, 'bold'), fg="#ffffff", bg="#16213e")
name_label.pack(anchor="w")
balance_label = tk.Label(customer_info, textvariable=balance_var, font=('Arial', 12), fg="#00d4aa", bg="#16213e")
balance_label.pack(anchor="w")
biometric_label = tk.Label(customer_info, textvariable=biometric_status_var, font=('Arial', 10), fg="#ffa500", bg="#16213e")
...
subtotal_display = tk.Label(bottom_frame, textvariable=subtotal_var, font=('Arial', 16, 'bold'), fg="#ffffff", bg="#1a1a2e")
subtotal_display.pack(pady=10)
submit_button = tk.Button(bottom_frame, text="🛒 Submit Order", command=lambda: submit_order(), bg="#00d4aa", fg="#1a1a2e", font=("Arial", 14, "bold"), relief="raised", bd=3, pady=10)
...
def create_menu_item_widget(parent, item):
    ...
    name_label = tk.Label(details_frame, text=item["name"], font=('Arial', 14, 'bold'), fg="#ffffff", bg="#16213e")
    name_label.pack(anchor="w")
    price_label = tk.Label(details_frame, text=f"Rs. {item['price']:.2f}", font=('Arial', 12, 'bold'), fg="#00d4aa", bg="#16213e")
    price_label.pack(anchor="w")
    stock_label = tk.Label(details_frame, text=f"Stock: {item['stock']}", font=('Arial', 10), fg="#cccccc", bg="#16213e")
    ...
    btn_frame.pack()
    minus_btn = tk.Button(btn_frame, text="−", command=decrement, width=3, bg="#ff6b6b", fg="white", font=("Arial", 12, "bold"), relief="raised")
    minus_btn.pack(side="left", padx=2)
    qty_label = tk.Label(btn_frame, textvariable=qty_var, width=4, fg="#ffffff", bg="#16213e", font=("Arial", 12, "bold"))
    qty_label.pack(side="left", padx=5)
    plus_btn = tk.Button(btn_frame, text="+", command=increment, width=3, bg="#00d4aa", fg="white", font=("Arial", 12, "bold"), relief="raised")
...
def submit_order():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if order_id:
                biometric_status_var.set("✅ Order placed! Take your order and enter the fingerprint...")
...
def initiate_biometric_authentication(order_id):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            root.after(0, lambda: biometric_status_var.set(f"👆 Queued for fingerprint scan (Queue size: {biometric_queue.qsize()})"))
...
def start_biometric_trigger_server(email, order_id, auth_header):
    ...
    with server_lock:
        ...
        try:
                ...
                def do_POST(self):
                    try:
                        ...
                        if self.path == '/trigger-biometric':
                            ...
                            root.after(0, lambda: biometric_status_var.set(f"👆 Queued for fingerprint scan (Queue size: {queue_size})"))
                            ...
                            root.after(0, lambda: messagebox.showerror("Biometric Error", error_msg))
                        else:
            ...
            root.after(0, lambda: messagebox.showerror("Error", f"Could not start biometric server: {str(e)}"))
...
def process_biometric_queue():
    while True:
        try:
            ...
            root.after(0, lambda: messagebox.showerror("Error", f"Error processing biometric queue: {str(e)}"))

def start_biometric_scanning(email, order_id, auth_header=None):
    try:
        print("DEBUG: Starting biometric scanning...")
        biometric_status_var.set("👆 Take your order and enter the fingerprint...")
        root.update()
        ...
        if esp32_res.status_code == 200:
            biometric_status_var.set("✅ Fingerprint scan initiated successfully!")
            root.after(500, lambda: [reset_to_main_screen(), stop_biometric_trigger_server()])
        else:
            biometric_status_var.set(f"❌ ESP32 error: HTTP {esp32_res.status_code}")
            messagebox.showerror("Biometric Error", f"ESP32 returned error: {esp32_res.text}")
            stop_biometric_trigger_server()
    except requests.exceptions.RequestException as e:
        print(f"DEBUG: Network error during biometric scanning: {e}")
        biometric_status_var.set("❌ Network error during fingerprint scan.")
        messagebox.showerror("Network Error", f"Could not connect to ESP32: {str(e)}")
        stop_biometric_trigger_server()
    except Exception as e:
        print(f"DEBUG: Unexpected error during biometric scanning: {e}")
        biometric_status_var.set("❌ Error during fingerprint scan.")
        messagebox.showerror("Error", f"Unexpected error during biometric scanning: {str(e)}")
        stop_biometric_trigger_server()
...
def rfid_loop():
    while True:
        try:
            ...
            time.sleep(0.5)  # Reduced delay for faster RFID scanning
        except Exception as e:
