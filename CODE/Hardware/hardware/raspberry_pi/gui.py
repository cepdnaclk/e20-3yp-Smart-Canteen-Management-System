...
# --- API URLs ---
PROFILE_URL = "http://192.168.1.101:8081/api/"
MENU_URL = "http://192.168.1.101:8081/api/menu-items"
ORDER_URL = "http://192.168.1.101:8081/api/orders/place"
...
def submit_order():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if order_id:
                # Proceed to call biometric endpoint
...
def rfid_loop():
    while True:
        try:
            # Read RFID card: id = numeric UID, text = stored string
            id, text = reader.read()
            rfid_text = text.strip()  # The text stored on the card
            if rfid_text:
                print(f"Read RFID card text: {rfid_text}")
