...
style.configure('Custom.TProgressbar', troughcolor=BACKGROUND_COLOR, background=SUCCESS_COLOR)
...
def fade_frame_out(frame, alpha=1.0, callback=None):
    if alpha > 0:
        ...
        frame.after(50, lambda: fade_frame_out(frame, alpha - 0.1, callback))
    else:
        frame.pack_forget()
...
def fade_frame_in(frame):
    ...
    def increase_alpha(alpha=0.1):
        if alpha < 1.0:
            frame.winfo_toplevel().attributes('-alpha', alpha)
            frame.after(50, lambda: increase_alpha(alpha + 0.1))
        else:
            frame.winfo_toplevel().attributes('-alpha', 1.0)
    increase_alpha()
...
def load_image_from_url(image_path, size=(100, 100)):
...
def create_placeholder_image(size=(100, 100)):
    try:
