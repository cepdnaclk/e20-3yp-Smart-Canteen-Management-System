...
class _OrderManagementScreenState extends State<OrderManagementScreen>
  ...
  Timer? _autoRefreshTimer; // ✨ ADD: Timer for auto-refreshing
  ...
  void initState() {
    ...
    // ✨ ADD: Start the auto-refresh timer. You can adjust the duration.
    _autoRefreshTimer = Timer.periodic(const Duration(seconds: 15), (timer) {
      if (mounted) {
        _fetchAllOrders(isAutoRefresh: true);
      }
    });
  }
  ...
  void dispose() {
    _tabController.dispose();
    _autoRefreshTimer?.cancel(); // ✨ ADD: Stop the timer to prevent memory leaks
    super.dispose();
  }

  // ✨ UPDATE: The method now accepts a flag to prevent the loading spinner on background refreshes.
  Future<void> _fetchAllOrders({bool isAutoRefresh = false}) async {
    // If it's a background refresh, don't show the main loading indicator.
    if (!isAutoRefresh) {
    ...
    try {
      ...
      if (mounted && !isAutoRefresh) {
        NotificationUtils.showAnimatedPopup(context, title: "Error", message: "Could not fetch orders.", isSuccess: false);
      }
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  ...
  Future<void> _updateOrderStatus(Order order, int index, String action) async {
    ...
    if (action == 'accept' || action == 'decline') {
      sourceList = _pendingOrders;
      listKey = _pendingListKey;
    } else if (action == 'complete') {
      sourceList = _acceptedOrders;
      listKey = _acceptedListKey;
    } else {
      return;
    }
    ...
    listKey.currentState?.removeItem(
      index,
          (context, animation) => _buildRemovedItem(order, animation),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (action == 'accept') _fetchOrders('accepted');
        if (action == 'complete') _fetchOrders('completed');

      } else {
        throw Exception('Failed to $action order');
      }
    } catch (e) {
      if (mounted) NotificationUtils.showAnimatedPopup(context, title: 'Error', message: 'Failed to update order. Reverting.', isSuccess: false);
      setState(() {
        sourceList.insert(index, order);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // ✨ REMOVED: The FloatingActionButton is no longer needed.
    );
  }
