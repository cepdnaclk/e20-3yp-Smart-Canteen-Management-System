...
class CartProvider with ChangeNotifier {
  ...
  // ✨ FIX: This method was missing from your file.
  /// Fetches all menu items from the API and populates the internal map.
  /// This should be called once when the app starts.
  Future<void> initializeMenuItems() async {
    if (_menuItemsMap.isNotEmpty) return;

    try {
      final token = await _storage.read(key: 'jwt_token');
      // This endpoint should return ALL menu items for the provider to have a complete list.
      ...
      if (response.statusCode == 200) {
        ...
        setMenuItems(items);
        print('CartProvider initialized with ${_menuItemsMap.length} menu items.');
      }
    } catch (e) {
      print('Failed to initialize menu items in CartProvider: $e');
    }
  }
  ...
  String getItemNameById(String id) {
    try {
      ...
      final menuItem = _menuItemsMap[itemId];
      if (_menuItemsMap.isEmpty) return 'Loading...';
