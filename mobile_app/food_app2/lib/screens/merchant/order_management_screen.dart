...
class _OrderManagementScreenState extends State<OrderManagementScreen> with SingleTickerProviderStateMixin {
  ...
  void _animateItemRemoval(GlobalKey<AnimatedListState> key, Order order, int index) {
  ...
  Future<void> _updateOrderStatus(Order order, int index, String action) async {
    // Optimistically remove the item from its current list
    if (action == 'accept' || action == 'decline') {
      _animateItemRemoval(_pendingListKey, order, index);
      _pendingOrders.removeAt(index);
    } else if (action == 'complete') {
      _animateItemRemoval(_acceptedListKey, order, index);
      _acceptedOrders.removeAt(index);
    }
    ...
    try {
      ...
      if(mounted) NotificationUtils.showAnimatedPopup(context, title: 'Error', message: 'Failed to update order status. Reverting.', isSuccess: false);
      _fetchAllOrders(); // Re-fetch all orders to revert UI on failure
    }
  }

  // --- NEW --- Function to delete a completed order from the view
  void _deleteCompletedOrder(Order order, int index) {
    setState(() {
    ...
    NotificationUtils.showAnimatedPopup(
      context,
      title: 'Order Cleared',
      message: 'Order #${order.id} has been removed from the list.',
      isSuccess: true,
    );
    // Note: This does not delete the order from the database, only from the UI.
    // To permanently delete, you would need a DELETE API call here.
  }
    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        children: [
          ...
          // --- NEW --- Pass the delete callback to the completed orders list
          _buildOrderList(
            _completedOrders,
            onDelete: _deleteCompletedOrder,
          ),
        ],
      ),
      floatingActionButton: FloatingActionButton(
  ...
  // --- NEW --- Update _buildOrderList to accept the onDelete callback
  Widget _buildOrderList(List<Order> orders, {Function(Order, int)? onDelete}) {
    if (orders.isEmpty) {
      return Center(child: Text('No orders in this category.', style: TextStyle(color: Colors.grey.shade600)));
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return OrderCard(
          order: order,
          onDelete: onDelete != null ? () => onDelete(order, index) : null,
        );
      },
    );
  }
